...
import { findCities, findType, transBrnads, transModels, transMonths, transYears } from '../../utils/transforms.data';
...
const AddCarScreen = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!price){
      tempErrors.price = "Le prix est requis";
      isValid = false;
    }
    if (!sellerTypeId){
      tempErrors.sellerType = "Type requis";
      isValid = false;
    }
    if (!sellerNameId){
      tempErrors.sellerName = "Nom requis";
      isValid = false;
    }
    if (!phoneRaw){
    ...
    if (!marqueId){
      tempErrors.marque = "Marque requise";
      isValid = false
    }
    if (!modelId){
      tempErrors.model = "Modèle requis";
      isValid = false;
    }
    if (!yearId){
      tempErrors.year = "Année requise";
      isValid = false;
    }
    if (!picture){
      tempErrors.picture = "Photo requise";
      isValid =false;
    }
    if(!email){
      tempErrors.email = "Email est requise";
      isValid = false;
    }
    if(!city) {
      tempErrors.city = "la ville est requise";
      isValid = false;
    }
    if(!address){
      tempErrors.address = "l'adress est requise"
      isValid = false;
    }
    if(!monthId){
       tempErrors.monthId = "le mois est requise";
       isValid = false;
    }
    if(!km){
       tempErrors.km = "kelimétrage est requise";
       isValid = false
    }
    if(!selCity) {
      tempErrors.selCity = "La ville est requise";
      isValid = false;
    }
    if(!( /^[^\s@]+@[^\s@]+\.[^\s@]+$/).test(email)) Toast.error("Format d'email invalide")
  ...
  const handleSubmit = () => {
    const carDataPayload:RegisterInput = {
        ...
        car: {
            ...
            id_brand: marqueId,
            id_model: modelId,
            city:findCities(city as any) as any,

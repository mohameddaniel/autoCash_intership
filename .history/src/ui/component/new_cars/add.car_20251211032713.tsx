...
const AddCarScreen = () => {
  ...
  // Voiture
  const [marque, setMarque] = useState<number>();
  ...
  useEffect(() => {
    dispatch(fetchModel(marque as any))
  },[marque])
   
  useEffect(() => {
    dispatch(fetchMonths(model as any))
    dispatch(fetchYears(model as any))
  },[model])
console.log("valeur",model)

...
const SmartCombobox = ({
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter(item => item?.name?.toLowerCase().includes(lowerValue));
  }, [data, searchText]);
  


  const handleSelect = useCallback((item: NamesType) => {
    setSearchText(item?.id as any)
    onChange(item?.name);
    Keyboard.dismiss();
    toggleList(false);
  }, [onChange]);
  ...
  return (<View style={[styles.wrapper, style, { zIndex }]}> 
      ...
      <View style={styles.relativeContainer}>
        ...
        <View style={[
          ...
          <TextInput
            ref={inputRef}
            // CHANGÉ : Affiche le searchText (qui est soit le nom sélectionné, soit la saisie)
            value={searchText}
            onChangeText={(text) => {
              setSearchText(text); // Met à jour l'état de recherche local
              // Réinitialise la sélection si l'utilisateur efface tout
              if (text === '' && value) {
                onChange(undefined);
              }
            ...
            onFocus={() => {
              toggleList(true);
              // Optionnel: réinitialiser le champ si la valeur est déjà sélectionnée
              // if (selectedItem) setSearchText(''); 
            }}
          ...
          <View style={styles.dropdown}>
            ...
            >
              {filteredData.map((item) => {
                 // CHANGÉ : La vérification de la sélection se fait sur l'ID
                 const isSelected:boolean = item.id === value; 
                 return (
                  <TouchableOpacity 
                    key={item.id} // Utiliser l'ID comme clé
                    style={[styles.item, isSelected && styles.itemSelected]} 
                    onPress={() => handleSelect(item)} // Passe l'objet complet
                    activeOpacity={0.7}
                  >

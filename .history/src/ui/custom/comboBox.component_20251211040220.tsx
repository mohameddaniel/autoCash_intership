...
interface SmartComboboxProps {
  ...
  onChange: (id: number | string | undefined,value?:string) => void; 
  placeholder?: string;
  error?: string;
  style?: ViewStyle;
  zIndex?: number; 
...
const SmartCombobox = ({
  ...
  const [searchText, setSearchText] = useState(selectedItem ? selectedItem?.name?.toString() : '');


  React.useEffect(() => {
      setSearchText(selectedItem ? selectedItem?.name?.toString() : '');
  }, [selectedItem]);
  ...
  const handleSelect = useCallback((item: NamesType) => {
    setSearchText(item?.name?.toString()); 
